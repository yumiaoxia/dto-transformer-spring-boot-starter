...
public class DtoTransFormer {
    ...
    private static Class<?> getViewClass() {
        RequestDtoInterceptor interceptor = SpringUtils.getBean("requestDtoInterceptor", RequestDtoInterceptor.class);
        return interceptor.getViewClass();
    }
    ...
    public static class PageHelper<R> {
        ...
        public final <T> Page<R> apply(Page<T> tPage) {
            ...
            List<R> resultContents = contents.stream().map(content -> DtoAssembleUtils.assemble(dtoClass, getViewClass(), content)).collect(Collectors.toList());
    ...
    public static class Helper<R> {
        ...
        public final <T> R apply(T... ts) {
            return DtoAssembleUtils.assemble(dtoClass, getViewClass(), ts);
        }
    }
    ...
    public static class ListHelper<R> {
        ...
        public <T> List<R> apply(Collection<T> ts) {
            return ts.stream().map(t -> DtoAssembleUtils.assemble(dtoClass, getViewClass(), t)).collect(Collectors.toList());
        }
    }
